...
class Archive_Creation_Job extends \WP_Background_Process {
	...
	public function __construct() {
		...
		$this->task_list = apply_filters( 'simplerstatic.archive_creation_job.task_list', array(), $this->options->get( 'delivery_method' ) );
	...
	protected function error_occurred( $wp_error ) {
		...
		$message = sprintf( __( "An error occurred: %s", 'simplerstatic' ), $wp_error->get_error_message() );
		$this->save_status_message( $message, 'error' );
		return 'cancel';
	}
