 ...
 * to the SimplerStatic\Url_Response that is passed into it: URLs in the body of
...
class Url_Extractor {
    ...
    private function extract_urls_and_update_tag( &$tag, $tag_name, $attributes ) {
        ...
        foreach ( $attributes as $attribute_name ) {
            if ( isset( $tag->$attribute_name ) ) {
                ...
                Util::debug_log( "attribute value $attribute_value" );
    ...
    private function convert_url( $url ) {
        ...
        } elseif ( $this->options->get( 'destination_url_type' ) == 'relative' ) {
            $url = $this->convert_relative_url( $url );
        } elseif ( $this->options->get( 'destination_url_type' ) == 'offline' ) {
