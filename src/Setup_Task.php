...
class Setup_Task extends Task {
    ...
    public static function add_origin_and_additional_urls_to_db( string $additional_urls ) {
        ...
        $static_page->set_status_message( __( 'Origin URL', 'simplerstatic' ) );
        ...
        foreach ( $urls as $url ) {
            if ( Util::is_local_url( $url ) ) {
                ...
                $static_page->set_status_message( __( 'Additional URL', 'simplerstatic' ) );
                $static_page->found_on_id = 0;
                $static_page->save();
            }
        }
        ...
        foreach ( Util::string_to_array( $additional_files ) as $item ) {
            ...
            if ( file_exists( $item ) ) {
                if ( is_file( $item ) ) {
                    ...
                    $static_page->set_status_message( __( 'Additional File', 'simplerstatic' ) );
                    ...
                    foreach ( $iterator as $file_name => $file_object ) {
                        ...
                        $static_page = Page::query()->find_or_initialize_by( 'url', $url );
                        $static_page->set_status_message( __( 'Additional Dir', 'simplerstatic' ) );
                        $static_page->found_on_id = 0;
                        $static_page->save();
                    }
                }
